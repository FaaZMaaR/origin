...
public:
	...
	double mean(size_t b, size_t e) {
		if (b >= e || e > _size) throw std::invalid_argument("Incorrect indexes!");
		double sum = 0;
	...
	size_t checkSum() {
		if (typeid(T) != typeid(bool) && typeid(T) != typeid(int)) throw std::bad_typeid();
		else if (typeid(T) == typeid(int)) {
			for (T v : extended_array) {
				if (v != 0 && v != 1) throw std::logic_error("ExtArray<int> can only contain 0 and 1!");
			}
		}
		size_t sum = 0;
		for (T v : extended_array) {
